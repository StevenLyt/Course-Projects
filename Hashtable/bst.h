...
protected:
    ...
    void helper(Node<Key, Value>* curr);                                      // recursive delete
    bool helper_balanced(Node<Key, Value>* curr) const;                       // resursively determine balance
    int helper_height(Node<Key, Value>* root, int recursionDepth = 1) const;  // get tree height
...
typename BinarySearchTree<Key, Value>::iterator& BinarySearchTree<Key, Value>::iterator::operator++()
{
    ...
    // have no right child
    while (current_->getParent() != NULL && current_->getParent()->getLeft() != current_)
    {
...
template<typename Key, typename Value>
void BinarySearchTree<Key, Value>::helper(Node<Key, Value>* curr)
{
    if (curr == NULL)
    {
        return;
...
template<typename Key, typename Value>
int BinarySearchTree<Key, Value>::helper_height(Node<Key, Value>* root, int recursionDepth) const
{
    if (root == nullptr)
    {
        return 0;
...
template<typename Key, typename Value>
bool BinarySearchTree<Key, Value>::helper_balanced(Node<Key, Value>* curr) const
{
    if (curr == NULL)
    {
        return true;
    ...
    if (helper_balanced(left) && helper_balanced(right) && abs(helper_height(left) - helper_height(right)) <= 1)
    {
        return true;
    }
    else
